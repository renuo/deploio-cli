...
module Deploio
  module Commands
    class MySQLBackups < Thor
      include SharedOptions

      namespace "ms:backups"

      DEDICATED_KIND = "MySQL"
      ECONOMY_KIND = "MySQLDatabase"

      desc "capture NAME", "Capture a new backup for the specified MySQL database"
      def capture(name)
      ...
      desc "list NAME", "List the available backups for the specified MySQL database"
      def list(name)
      ...
      desc "download NAME [--output destination_path]", "Download the latest backup for the specified MySQL database instance"
      ...
      def backup_service_for(name)
        ...
        case data["kind"]
        when DEDICATED_KIND
          MysqlBackupService.new(data: data, name: name, dry_run: @nctl.dry_run)
        when ECONOMY_KIND
