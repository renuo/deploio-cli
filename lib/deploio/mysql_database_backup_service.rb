...
module Deploio
  class MysqlDatabaseBackupService
    ...
    def capture
      raise Deploio::UnsupportedBackupOperationError,
            "'#{@db_ref.full_name}' is an economy-tier database. Those are backed up automatically " \
              ...
              "Use 'deploio ms backups list #{@name}' to see the available backups."
    end
    ...
    def own_backup?(object_name)
      ...
      object_name.start_with?("MySQLDatabase-#{instance_name}-")
    end
    ...
    def backup_bucket_for_database?(bucket)
      ...
      metadata["name"].to_s.match?(/\Amysqldatabase-#{Regexp.escape(@db_ref.database_name)}-[0-9a-f]{7}\z/)
    end
