...
class MsDatabaseRefTest < Minitest::Test
  def test_parses_full_name_from_available_databases
    available = {
      "n10518-maindb" => {project_name: "n10518", database_name: "maindb"}
    }
    db_ref = Deploio::MsDatabaseRef.new("n10518-maindb", available_databases: available)
  ...
  def test_full_name_method
    available = {
      "myproject-mysql" => {project_name: "myproject", database_name: "mysql"}
    }
    db_ref = Deploio::MsDatabaseRef.new("myproject-mysql", available_databases: available)

    assert_equal "myproject-mysql", db_ref.full_name
  end
  ...
  def test_to_s_returns_full_name
    ...
    db_ref = Deploio::MsDatabaseRef.new("proj-db", available_databases: available)
  ...
  def test_equality_comparison
    ...
    db_ref1 = Deploio::MsDatabaseRef.new("proj-db1", available_databases: available)
    db_ref1_copy = Deploio::MsDatabaseRef.new("proj-db1", available_databases: available)
    db_ref2 = Deploio::MsDatabaseRef.new("proj-db2", available_databases: available)
  ...
  def test_raises_error_when_database_not_found
    ...
    error = assert_raises(Deploio::MsDatabaseNotFoundError) do
      Deploio::MsDatabaseRef.new("nonexistent-db", available_databases: available)
    end

    assert_match(/Database not found: 'nonexistent-db'/, error.message)
    assert_match(/Run 'deploio ms' to see available MySQL databases/, error.message)
  end
  ...
  def test_suggests_similar_database_names
    ...
    error = assert_raises(Deploio::MsDatabaseNotFoundError) do
      Deploio::MsDatabaseRef.new("myproject-maindv", available_databases: available)
    end
  ...
  def test_raises_error_when_no_databases_available
    error = assert_raises(Deploio::MsDatabaseNotFoundError) do
      Deploio::MsDatabaseRef.new("some-db", available_databases: {})
    end
