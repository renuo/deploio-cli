...
class MysqlDatabaseBackupServiceTest < Minitest::Test
  ...
  def build_service(buckets:, objects: [], database_name: "main", data: database_data)
    ...
    service = Deploio::MysqlDatabaseBackupService.new(
  ...
  def test_finds_the_backup_bucket_named_after_the_database
    service, rclone = build_service(
      buckets: [
        plain_bucket("swissict-develop"),
        backup_bucket("mysqldatabase-main-cffe5c3")
      ],
      objects: [object("MySQLDatabase-#{INSTANCE_NAME}-2026-07-30-0224.sql.zst", "2026-07-30T02:24:22Z")]
    )

    assert_equal 1, service.backups.size
    assert_equal ["mysqldatabase-main-cffe5c3"], rclone.listed
  end

  def test_ignores_buckets_not_owned_by_a_backup_schedule
    service, = build_service(buckets: [plain_bucket("mysqldatabase-main-cffe5c3")])
  ...
  def test_does_not_match_the_bucket_of_a_similarly_named_database
    ...
    service, = build_service(
      buckets: [backup_bucket("mysqldatabase-labels-main-be40f63")],
  ...
  def test_matches_a_database_whose_name_contains_a_hyphen
    ...
    assert_equal ["mysqldatabase-labels-main-be40f63"], rclone.listed
  end
  ...
  def test_keeps_only_objects_belonging_to_this_database
    service, = build_service(
      ...
      objects: [
        ...
        object("MySQLDatabase-other_instance-2026-07-30-0224.sql.zst", "2026-07-30T02:24:22Z"),
        object("some-unrelated-file.txt", "2026-07-30T03:00:00Z")
      ]
    )

    assert_equal ["MySQLDatabase-#{INSTANCE_NAME}-2026-07-29-0224.sql.zst"], service.backups.map { |b| b["Name"] }
  end
  ...
  def test_orders_backups_newest_first
    service, = build_service(
      buckets: [backup_bucket("mysqldatabase-main-cffe5c3")],
      objects: [
        object("MySQLDatabase-#{INSTANCE_NAME}-2026-07-28-0227.sql.zst", "2026-07-28T02:27:37Z"),
  ...
  def test_download_raises_when_the_bucket_holds_no_backups
    service, rclone = build_service(buckets: [backup_bucket("mysqldatabase-main-cffe5c3")])
